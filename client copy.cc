        ...
        for (int n = 0; n < nfds; ++n) {
            if (events[n].data.fd == client_fd) {
                if (events[n].events & EPOLLIN){
                    ...
                    while(true){
                        auto retval = recvmmsg(client_fd, msgs, 500, 0, NULL);
                        if (retval == -1){
                            if (errno == EAGAIN) {
                                break;
                            }
                        ...
                        for (auto index = 0; index < retval; index++){
                            ...
                            if ((ack_pkt_index != 0) && (ack_pkt_index != (retval - 1)) && index == (retval - 1)){
                                auto dmludpread = dmludp_conn_recv(dmludp_connection, static_cast<uint8_t *>(msgs[ack_pkt_index].msg_hdr.msg_iov->iov_base), read);
                                ssize_t dmludpwrite = dmludp_conn_send(dmludp_connection, out, sizeof(out));
                                ssize_t socketwrite = ::send(client_fd, out, dmludpwrite, 0);
                                if(dmludp_conn_receive_complete(dmludp_connection)){
                                    std::cout<<recv_time++<<" time receive complete"<<std::endl;
